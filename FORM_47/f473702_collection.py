...
for file in os.listdir('.'):
    ...
    for i in range(NUMBER_OF_CATEGORIES):
        ...
        category_df = pd.DataFrame(columns=["nregion_hospital", "nrural", "ndoctors"])

        category_df["region"] = regions.values
        category_df["year"] = year
        category_df["category"] = category_title
        
        category_df["nregion_hospital"] = category_data.iloc[:, 0].values
        category_df["nrural"] = category_data.iloc[:, 1].values
        category_df["ndoctors"] = category_data.iloc[:, 3].values

        dataframes.append(category_df)

full_data_frame = pd.concat(dataframes)
full_data_frame.to_csv("F473702.csv", index=False)
